...
;;      :valid? false})</code></pre><p>Two scalar predicates in the specification applied to the one scalar datum. <code>:chocolate</code> is a keyword, but not a qualified keyword. Next, we&apos;ll see how to validate multiple scalars with multiple scalar predicates.</p><pre><code>(validate-scalars #{:chocolate :vanilla :strawberry}
&nbsp;                 #{keyword? qualified-keyword?})
