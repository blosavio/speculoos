...
[:body
 ...
 (panel
  ...
  [:div.note "Being able to validate Clojure data enables us to check the usage and behavior of functions."
 ...
 (panel
  ...
  [:div.note "When we invoke a function with a series of arguments, that series of values forms a sequence, which Speculoos can validate like any other heterogeneous, arbitrarily-nested data structure. Speculoos offers " [:a {:href "#explicit"} "a trio"] " of function-validating functions with differing levels of explicitness. We'll be primarily using " [:code "validate-fn-with"] " because it is the most explicit of the trio, and we can most easily observe what's going on."])
 ...
 (panel
  ...
  [:div.note "Let's pretend we want to validate the arguments to a function " [:code "sum-three"] " that expects three integers and returns their sum."
 ...
 (panel
  ...
  [:div.note "…validating scalars with " [:code "validate-scalars"] " and keeping only the invalids."])
 ...
 (panel
  ...
  [:p "See " [:em "Speculoos Screencast 4 — Validating Collections"] "."]

  [:div.note "Okay, we see that term " [:em "scalar"] " buzzing around, so there must be something else about validating collections. Yup. We can also validate collection properties of the argument sequence. Let's specify that the argument sequence must contain three elements, using a custom collection predicate."
 ...
 (panel
  ...
  [:div.note "Next, we'll associate that collection specification into our function specification map at " [:code ":speculoos/arg-collection-spec"] " and invoke " [:code "validate-fn-with"] " with three valid arguments." "The argument sequence satisfies our collection specification, so " [:code "sum-three"] " returns the expected value."])
 ...
 (panel
  ...
  [:div.note " Now let's repeat, but with an additional argument that causes the argument list to violate its collection predicate.""This four-element argument list, " [:code "[1 20 300 4000]"] ", failed to satisfy our " [:code "count-3?"] " collection predicate, so " [:code "validate-fn-with"] " emitted a validation report."
 ...
 (panel
  ...
  [:div.note "Let's get fancy and combine an argument scalar specification and an argument collection specification. Outside of the context of checking a function, that " [:a {:href "#combo"} "combo validation"] " would look like this."
 ...
 (panel
  ...
  [:div.note "Speculoos can also validate values returned by a function. Reusing our " [:code "sum-three"] " function, and going back to valid inputs, we can associate a " [:strong "ret"] "urn " [:strong "scalar"] " " [:strong "spec"] "ification into " [:code "validate-fn-with"] "'s specification map to key " [:code ":speculoos/ret-scalar-spec"] ". Let's stipulate that the function returns an integer. Here's how we pass that specification to " [:code "validate-fn-with"] "."])
 ...
 (panel
  ...
  [:div.note "And now, the function return validation."
 ...
 (panel
  ...
  [:div.note "What happens when the return value is invalid? Instead of messing up " [:code "sum-three"] "'s definition, we'll merely alter the scalar predicate. Instead of an integer, we'll stipulate that " [:code "sum-three"] " returns a string with scalar predicate " [:code "string?"] "."
 ...
 (panel
  ...
  [:div.note "Let's see how to validate a function when the return value is a collection of scalars. We'll write a new function that returns four scalars: the three arguments and their sum."

   [:p "Our enhanced function now returns a vector of four elements."]])
 ...
 (panel
  ...
  [:div.note  "Let's remind ourselves how we'd manually validate that return value. If we decide we want " [:code "enhanced-sum-three"] " to return four integers, the scalar specification would look like this." "And the manual validation would look like this."
 ...
 (panel
  ...
  [:div.note "Since we fed " [:code "validate-fn-with"] " a specification that happens to agree with those arguments, " [:code "enhanced-sum-three"] " returns its computed value, " [:code "[1 20 300 321]"] "."])
 ...
 (panel
  ...
  [:div.note "Let's toss that collection specification at " [:code "validate-with-fn"] " and have it apply to " [:code "enhanced-sum-three"] "'s return value, which won't satisfy. We pass the " [:strong "ret"] "urn " [:strong "collection spec"] "ification by associating it to the key " [:code ":speculoos/ret-collection-spec"] "."
 ...
 (panel
  ...
  [:div.note "Now that we've seen the combo validation done manually, let's validate " [:code "enhanced-sum-three"] "'s return in the same way. Here's where we see why to organize the specifications in a container instead of passing them as individual arguments: it keeps our invocation neater."
 ...
 (panel
  ...
  [:div.note "Okay. I think we're ready to put together all four different function validations we've so far seen. We've seen…"])
 ...
 (panel
  ...
  [:p "argument scalar specification: "[:code "[string? int? symbol?]"]]

  (prettyfy-form-prettyfy-eval "(defn length-2? [v] (= 2 (count v)))")

  [:p "argument collection specification: " [:code "[length-2?]"]]

  [:p "return scalar specification:" [:code "[char? int? int? boolean?]"]]

  [:p "return collection specificaiton: "[:code "[list?]"]]

  [:div.note "And we've seen how to combine both function argument validations, and how to combine both function return validations. Now we'll combine all four validations into one " [:code "validate-fn-with"] " invocation."
 ...
 (panel
  ...
  [:pre [:code "{:speculoos/arg-scalar-spec     [string? int? symbol?]\n :speculoos/arg-collection-spec [#(= 2 (count %))]\n :speculoos/ret-scalar-spec     [char? int? int? boolean?]\n :speculoos/ret-collection-spec [list?]}"]]
   ...
   [:div.note "Altogether, those four specification are organized like this."])
 ...
 (panel
  ...
  [:div.note "It's time to see what we've assembled."
 ...
 (panel
  ...
  [:p "A " [:em "reverse"] " function should turn this…"]
 ...
 (panel
  ...
  [:div.note " I must've written it before I had my morning coffee."
 ...
 (panel
  ...
  [:div.note "First, we'll write three " [:a {:href "#relationship"} "relationship functions"] ". Relationship functions are a lot like predicates. They return a truthy or falsey value, but instead consume two things instead of one. The function's argument sequence is passed as the first thing and the function's return value is passed as the second thing."
 ...
 (panel
  ...
  [:code "{:speculoos/argument-return-relationships [{:path-argument [0]\n                                            :path-return []\n                                            :relationship-fn same-length?}]}"]

  [:div.note "We stuff all three of those items into a map, which will be used for a single relationship validation."
 ...
 (panel
  ...
  [:div.note "We're ready to validate."
 ...
 (panel
  ...
  [:div.note "Just for amusement, let's see what happens when we validate " [:code "clojure.core/reverse"] " with the exact same relationship specifications."
 ...
 (panel
  [:h3 "Specifying relationships between 'bare' argument scalars and 'bare' return scalars."]
  ...
  [:div.note "Not every function consumes a collection. Some functions consume a scalar value. Some functions return a scalar. And some functions have the audacity to do both. " [:code "validate-fn-with"] " can validate that kind of argument/return relationship. "
 ...
 (panel
  ...
  [:div.note "Now is a good time to write the buggy incrementing function."

   [:p "Looks plenty wrong. Let's see exactly how wrong."]])


 (panel
  [:h3 "Validating 'bare' argument/return relationship"]
  ...
  [:div.note [:code "buggy-inc"] "'s return value failed to satisfy both relationships with its argument. " [:code "-99"] " is not larger than " [:code "99"] ", nor is it what we'd get by adding one to " [:code "99"] "."
 ...
 (panel
  [:h3 "More specific paths for validating argument/retrurn relationships"]
 ...
 (panel
  ...
  [:div.note "Now, we insert those two specifications into a vector and associate that vector into the organizing map."])
 ...
 (panel
  ...
  [:div.note "All that remains is to consult " [:code "validate-fn-with"] " to see if the relationships are satisfied. First, we'll do " [:code "pull-n-put"] ", which should yield the intended results."
 ...
 (panel
  ...
  [:div.note "Now we'll validate " [:code "pull-n-whoops"] ", which does not produce correct results."
 ...
 (panel
  ...
  [:div.note "Just so we see an invalid result, we'll make the argument collection specification expect a list, even though we know we'll be passing a vector. And again, we must make the collection specification's shape mimic the data, so to mimic the argument sequence, it looks like this."])
 ...
 (panel
  ...
  [:div.note "We know that " [:code "broken-reverse"] " returns the input collection with " [:code "9999"] " conjoined. We'll write the return scalar specification to expect a string in the fourth slot, just so we'll see " [:code "44"] " fail to satisfy."])
 ...
 (panel
  ...
  [:div.note "And since we're expecting " [:code "broken-reverse"] " to return a vector, we'll write the return collection specification to expect a set."])
 ...
 (panel
  ...
  [:div.note "Finally, we've previously demonstrated that " [:code "broken-reverse"] " fails to satisfy the " [:code "reversed?"] " argument/return relationship specification. We'll pass " [:code "reversed?"] " the first argument and the entire return."])


 (panel
  [:h3 "Assembling five function validations: Gotta get organized"]

  [:pre (prettyfy-form-prettyfy-eval "{:speculoos/arg-scalar-spec [[any? any? decimal?]]
  ...
  [:div.note "We assemble all five of those specifications into the organizing map, and invoke " [:code "validate-fn-with"] "."])


 (panel
  [:h3 "Validating function five ways."]

  [:pre (prettyfy-form-prettyfy-eval "(validate-fn-with broken-reverse {:speculoos/arg-scalar-spec [[any? any? decimal?]]
                                                                 :speculoos/arg-collection-spec [[list?]]
                                                                 :speculoos/ret-scalar-spec [any? any? any? string?]
                                                                 :speculoos/ret-collection-spec [set?]
                                                                 :speculoos/argument-return-relationships [{:path-argument [0]
  ...
  [:div.note "We supplied five specifications, five datums failed to satisfy those specifications, and we receive five invalidation entries."
 ...
 (panel
  ...
  [:div.note "Speculoos consults the following defined group of keys in a specification map when it validates."])
 ...
 (panel
  ...
  [:div.note "The metadata specifications are passive and have no effect during normal invocation."
 ...
 (panel
  ...
  [:div.note "all recognized keys are removed"
   [:code "validate-fn"] " only interrupts when a predicate paired with a datum is not satisfied. If we remove all the specifications, there won't be any predicates. Let's remove " [:code "sum-three"] "'s metadata specifications with " [:code "unject-specs!"] "."
 ...
 (panel
  ...
  [:div.note "We can try a more involved example. Let's inject that " [:a {:href "#messy"} "messy ball"] " of metadata specifications into " [:code "broken-reverse"] "."
 ...
 (panel
  ...
  [:div.note "Finally, we validate " [:code "broken-reverse"] "."
 ...
 (panel
  [:h3 "Instrumenting functions: Implicit validaton"]
  ...
  [:div.side-by-side-container
   ...
   [:div.side-by-side
    ...
    [:pre
     ...
     [:code.eval ";; => ({:path [2], :datum 300.0, :predicate int?, :valid? false, :fn-spec-type :speculoos/argument}\n"
      ";;     {:path nil, :datum 321.0, :predicate int?, :valid? false, :fn-spec-type :speculoos/return})"]]]]

  [:div.note "Let's instrument " [:code "sum-three"] " and see what happens."
...
"]
 ...
 (panel
  ...
  [:div.note "When we want to revert " [:code "sum-three"] " back to normal, we " [:code "unstrument"] " it."
 ...
 (panel
  ...
  [:div.note "And finally, we execute the validation with " [:code "validate-higher-order-fn"]
 ...
 (whats-next-panel
  ...
  [:div.note "Function validation speaker note text..."])
 ]
