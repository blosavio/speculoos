...
[:body
 ...
 (panel
  ...
  [:div.note "A " [:em "path"] " is a sequence of indexes, keys, or identities that allow us refer to a single element buried within a nested data structure. For each level of nesting, we add an element to the path sequence. " [:code "clojure.core/get-in"] " illustrates how this works."])
 ...
 (panel
  ...
  [:div.note "…or, if we use a " [:a {:href "https://github.com/blosavio/fn-in"} "modified version"] " of " [:code "clojure.core/get-in"] ", nest a vector within a map within a list." [:code "102"] " is contained in three levels of nesting, so its path is comprised of three pieces." [:a {:href "https://github.com/blosavio/fn-in"} [:code "fn-in"]] ": Inspect, update, exchange, and remove elements from heterogeneous, arbitrarily-nested Clojure data structures."])
 ...
 (panel
  ...
  [:div.note "Must convert to vector so that `all-paths` can inspect it."])
 ...
 (panel
  ...
  [:div.note "There is nothing special about integers. " [:code "all-paths"] " will treat any element, scalar or collection, the same way. " [:em "Every element has a path."] " We could replace those integers with functions, un-nested in a vector…" "…or nested in a map…" "The important principle to remember is this: Every element, scalar and collection, of a heterogeneous, arbitrarily-nested data structure, can be assigned an unambiguous path, regardless of its container type."
   "Four elements, four paths. Three scalars (functions in this example), one collection."])


 (whats-next-panel
  mechanics-index
  [:div.note "What's next note within mechanics screencast..."])
 ]