[:body
 [:section#troubleshooting
  ...
  [:ul
   ...
   [:li [:p "Use the verbose functions. If we're using the high-level " [:code "valid-…?"] " function variants, we'll only see " [:code "true/false"] ", which isn't helpful when troubleshooting. The " [:code "validate-…"] " variants are chatty and will display everything it considered during validation."]]
   ...
   [:li
    ...
    [:div.no-display (require '[speculoos.function-specs :refer [validate-fn-with]])]
    [:pre
