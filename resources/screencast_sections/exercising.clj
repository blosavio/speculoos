...
[:body
 ...
 (panel
  ...
  [:div.note "Speculoos can also generate random scalar samples from predicate-like things, such as regular expressions and sets."
 ...
 (panel
  ...
  [:div.note "This document often uses 'basic' predicates like " [:code "int?"] " and " [:code "string?"] " because they're short to type and straightforward to understand. In real life, we'll want to specify our data with more precision. Instead of merely " [:em "An integer"] ", we'll often want to express " [:em "An even positive integer between ninety and one-hundred."] " To do that, we need to create custom generators."])


 (panel
  [:h3 "Sample generator to complement a predicate"]

  [:em "Integer in the nineties."]
  ...
  [:div.note "The custom generators we discussed in the previous subsection are merely floating around in the ether. To use them for exercising, we need to put those generators in a spot that Speculoos knows: the predicate's metadata."
 ...
 (panel
  ...
  [:div.note "To make the generator invocable, we'll wrap it in a function."])
 ...
 (panel
  ...
  [:div.note "Speculoos uses function metadata for two purposes, and it's important to keep clear in our minds which is which."])
 ...
 (panel
  ...
  [:div.note [:code "defpred"] " does indeed relieve us of some tedious keyboarding, but it offers another benefit. If we arrange the predicate definition according to " [:code "defpred"] "'s expectations, it can automatically create a random sample generator for that predicate. Let's see it in action and then we'll examine the details."
 ...
 (panel
  ...
  [:div.note "For " [:code "defpred"] " to do its magic, the predicate definition must follow a few patterns."
 ...
 (panel
  ...
  [:div.note "Speculoos exposes the internal tool " [:code "defpred"] " uses to create a generator, so we can inspect how it works. (I've lightly edited the output for clarity.)"
 ...
 (panel
  ...
  [:pre
   ...
   [:code.form ";; => (gen/such-that (fn [i] (and (even? i) (pos? i)))\n;;       gen/small-integer {:max-tries speculoos.utility/*such-that-max-tries*})  <output elided for brevity>"]]

  [:div.note  " Next, we'll add a couple of modifiers with " [:code "and"] ". " [:code "int?"] " is in the first clause. (Again, lightly edited.)"
 ...
 (panel
  ...
  [:div.note "Let's see what happens with an " [:code "or"] "."
 ...
 (panel
  ...
  [:div.note "When automatically creating random sample generators, " [:code "defpred"] " handles nesting up to two levels deep. Let's see how we might combine both " [:code "or"] " and " [:code "and"] ". We'll define a predicate that tests for either an odd integer, a string of at least three characters, or a ratio greater than one-ninth."
 ...
 (panel
  ...
  [:div.note "We defined scalar predicate " [:code "pred-with-incorrect-generator"] " to require an integer, but, using " [:code "defpred"] ", we manually created a generator that emits ratio values. Each of the generated samples fails to satisfy the " [:code "int?"] " predicate."
 ...
 (panel
  ...
  [:div.note "Pretend somebody hands us a specification. It might be useful to know if we need to write a random sample generator for any of the predicates it contains, or if Speculoos can find a generator for all of them, either in the collection of known predicates-to-generators associations, or in the predicates' metadata. " [:code "unfindable-generators"] " tells us this information."
 ...
 (panel
  ...
  [:div.note "Speculoos knows how to create random samples from all three of those predicate-like things, so " [:code "unfindable-generators"] " returns an empty vector, " [:em "nothing unfindable"] ". Now, let's make a scalar specification with three predicates that intentionally lack generators."
 ...
 (panel
  ...
  [:div.note "Speculoos can do three things with random sample generators."
 ...
 (panel
  ...
  [:div.note "Let's try another example. The scalar specification will be a map with three keys associated with predicates for a character, a set-as-a-predicate, and a regex-predicate."
 ...
 (panel
  ...
  [:pre
   ...
   [:code ";; => (gen/such-that (fn [s] (and (= 3 (count s)))) gen/string-alphanumeric) <output elided for brevity>"]]
  ...
  [:div.note "If our scalar specification contains custom predicates, we'll have to provide a little more information. We'll make another scalar specification containing a positive, even integer…"
 ...
 (panel
  ...
  [:div.note "Now that we have two scalar predicates with custom sample generators — one created by " [:code "defpred"] ", one created by us — we'll bring them together into a single scalar specification and invoke " [:code "data-from-spec"] "."])
 ...
 (panel
  ...
  [:div.note [:code "data-from-spec"] " generates a valid data set whose randomly-generated scalars satisfy the scalar specification. In fact, we can feed the generated data back into the specification and it ought to validate " [:code "true"] "."])
 ...
 (panel
  ...
  [:div.note "Perhaps it would be nice to do that multiple times in a row: generate some random data from a specification and feed it back into the specification to see if it validates. Don't go off and write your own utility. Speculoos can "  [:em "exercise"] " a scalar specification."
 ...
 (panel
  ...
  [:div.note "Sometimes it might be useful that a generated value be predictable. Perhaps we're writing documentation, or making a presentation, and we'd like the values to be aesthetically pleasing. Or, sometimes during development, it's nice to be able to quickly eyeball a known value."
 ...
 (panel
  ...
  [:div.note "The two exercising functions, " [:code "exercise"] " and " [:code "exercise-fn"] " both accept the " [:code ":canonical"] " option, as well."
 ...
 (panel
  ...
  [:div.note "Beyond the built-in canonical values, we can supply canonical values of our own choosing when we define a predicate. We can manually add the canonical values via " [:code "with-meta"] " or we can add a canonical value using " [:code "defpred"] " as an argument following a custom generator."
 ...
 (whats-next-panel
  exercising-index
  [:div.note "Exercising What's next speaker note..."])
 ]