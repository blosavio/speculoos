...
[:body
 ...
 (panel
  ...
  [:div.note
   [:p "…but Speculoos provides a function that does exactly that, both for scalars for scalars and for collections. Whereas the " [:code "validate-…"] " functions return a detailed validation report of every predicate+datum pair they see, the " [:code "valid-…?"] " variants provide a plain " [:code "true/false"] "."]])
 ...
 (panel
  ...
  [:div.note
   [:p "Let's look at a combo example. First, the 'plain' version. We validated the single vector, and only one out of the three scalars. " [:code "valid?"] " only considers paired elements+predicates, so it only validated " [:code "42"] " and the root vector. " [:code "valid?"] " ignored " [:code "\"abc\""] " and " [:code "22/7"] "."]])
 ...
 (panel
  ...
  [:div.note
   [:p "The thorough variant, " [:code "thoroughly-valid?"] ", however, does not ignore un-paired data elements. Even though both predicates, " [:code "int?"] " and " [:code "vector?"] ", were satisfied, " [:code "thoroughly-valid?"] " requires that all data elements be validated. Since " [:code "42"] " and " [:code "22/7"] " are un-paired, the entire validation returns " [:code "false"] "."]])
 ...
 (panel
  ...
  [:div.note
   [:p "Note: Thoroughly validating does not ensure any measure of correctness or rigor. 'Thorough' merely indicates that each element was exposed to " [:em "some"] " kind of predicate. That predicate could actually be trivially permissive. In this example, " [:code "any?"] " returns " [:code "true"] " for all values."]
 ...
 (panel
  ...
  [:p "Here are the general patterns regarding the function names." "'Plain' functions (i.e., " [:code "validate"] ", " [:code "valid?"] ", and " [:code "thoroughly-valid?"] ") perform a scalar validation, followed by performing a distinct collection validation, and returns a single comprehensive response that merges the results of both."]])
