...
[:body
 ...
 (panel
  ...
  [:div.side-by-side-container
   [:div.side-by-side (prettyfy-form-prettyfy-eval "(scalars-without-predicates [42 [\"abc\" 22/7]] [int?])" 50 40)]
   [:div.side-by-side (prettyfy-form-prettyfy-eval "(predicates-without-scalars [42] [int? string? ratio?])")]]

  [:div.note "Recall that Speculoos only validates using elements in the data and predicates in the specification located at identical paths. This duo of utilities tells us where we have unmatched scalars or unmatched predicates. The first of the duo tells us about un-paired scalars."
 ...
 (panel
  ...
  (prettyfy-form-prettyfy-eval "(predicates-without-scalars [42 \"abc\"] [int? [string? ratio?]])" 55 40)

  [:div.note "It is especially helpful for " [:a {:href "#troubleshooting"} "diagnosing surprising results"] ". Just because we put a predicate into the scalar specification doesn't force validation of a scalar that doesn't exist."
 ...
 (panel
  ...
  [:div.note "It's not difficult to neglect a predicate for a nested element within a collection specification, so Speculoos offers analogous utilities to highlight those possible issues."
 ...
 (panel
  ...
  [:div.note "Taking those ideas further, the " [:a {:href "#thorough"} [:em "thorough validation variants"]] " return " [:code "true"] " only if every scalar and every collection in data have a corresponding predicate in the scalar specification and the collection specification, respectively, and all those predicates are satisfied."
 ...
 (panel
  ...
  [:div.side-by-side-container
   [:div.side-by-side (prettyfy-form-prettyfy-eval "(spec-from-data [33 {:a :baz :b [1/3 false]} '(3.14 \\z)])" 65 25)]
   [:div.side-by-side (prettyfy-form-prettyfy-eval "(data-from-spec {:x int? :y [ratio? boolean?] :z (list char? neg-int?)} :random)" 65 25)]]

  [:div.note "I think of the next few utilities as " [:em "creative"] ", making something that didn't previously exist. We'll start with a pair of functions which perform complimentary actions."
 ...
 (panel
  ...
  [:div.note "Speaking of raw material, Speculoos also has a collection specification generator."
