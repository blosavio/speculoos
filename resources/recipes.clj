        ...
        (into [:article
              ...
              (section-nav
               [:section#valid-vs-validation
                ...
                [:pre
                 (print-form-then-eval "(require '[speculoos.core :refer [validate-scalar-spec valid-scalar-spec? only-valid only-invalid]])")
                 [:br]
                 (print-form-then-eval "(validate-scalar-spec [42 :foo \"abc\"] [int? keyword? boolean?])")]
                [:p "Speculoos provides some helper functions to filter the results."]
                [:pre
                 (print-form-then-eval "(only-valid (validate-scalar-spec [42 :foo \"abc\"] [int? keyword? boolean?]))")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec [42 :foo \"abc\"] [int? keyword? boolean?]))")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? keyword? string?])")
                 [:br]
               ...
               [:section#scalar-validation
                ...
                [:pre
                 (print-form-then-eval "(validate-scalar-spec [] [int?])")
                 (print-form-then-eval "(validate-scalar-spec '() [int?])")
                 (print-form-then-eval "(validate-scalar-spec {} [int?])")
                 (print-form-then-eval "(validate-scalar-spec #{} [int?])")]
                ...
                [:pre
                 (print-form-then-eval "(validate-scalar-spec [42 :foo \"abc\"] [])")
                 (print-form-then-eval "(validate-scalar-spec '(42 :foo \"abc\") [])")
                 (print-form-then-eval "(validate-scalar-spec {:a 42 :b :foo :c \"abc\"} [])")
                 (print-form-then-eval "(validate-scalar-spec #{42 :foo \"abc\"} [])")]

                [:h3 "Vectors"]
                [:p "Speculoos validates a vector's scalars in order."]
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42] [int?])")
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo] [int? keyword?])")
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? keyword? string?])")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec [42 :foo \"abc\"] [int? keyword? boolean?]))")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? (list 42) (list int?))")
                 (print-form-then-eval "(valid-scalar-spec? (list 42 :foo) (list int? keyword?))")
                 (print-form-then-eval "(valid-scalar-spec? (list 42 :foo \"abc\") (list int? keyword? string?))")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec (list 42 :foo \"abc\") (list int? keyword? boolean?)))")]

                [:h3 "Maps"]
                [:p "Speculoos validates a map's scalars by key."]
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? {:a 42} {:a int?})")
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} {:a int? :b symbol?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo :c \"abc\"} {:a int? :b symbol? :c string?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo :c \"abc\"} {:b symbol? :c string? :a int? })")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec {:a 42 :b 'foo :c \"abc\"} {:a int? :b boolean? :c string?}))")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? #{42} #{int?})")
                 (print-form-then-eval "(valid-scalar-spec? #{42 0 99} #{int?})")
                 (print-form-then-eval "(valid-scalar-spec? #{42 'foo 99} #{int?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? #{42} #{int? number? pos?})")
                 (print-form-then-eval "(valid-scalar-spec? #{42} #{int? string?})")
                 (print-form-then-eval "(valid-scalar-spec? #{42 99 -11} #{int? number? pos?})")
                 [:br]
                 (print-form-then-eval "(defn int-or-str? [x] ((some-fn int? string?) x))")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? #{42 \"abc\" 99} #{int-or-str?})")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? (repeat 42) [int?])")
                 (print-form-then-eval "(valid-scalar-spec? (repeat 42) [int? int? int?])")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec (repeat 42) [int? string? int?]))")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (cycle [42 :foo \"abc\"]) [int?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (cycle [42 :foo \"abc\"]) [int? keyword? string?])")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec (cycle [42 :foo \"abc\"]) [int? boolean? string?]))")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (range) [int?])")
                 (print-form-then-eval "(valid-scalar-spec? (range) [int? int? int?])")
                 (print-form-then-eval "(valid-scalar-spec? (range) [int? boolean? int?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (iterate inc 1) [int?])")
                 (print-form-then-eval "(valid-scalar-spec? (iterate inc 1) [int? int? int?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (iterate inc 1) [int? boolean? int?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (lazy-seq [1 2 3]) [int?])")
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] (cycle [int? keyword? string?]))")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] (cycle [int? boolean? string?]))")]
                ...
                [:pre
                 ...
                 (print-form-then-eval "(valid-scalar-spec? [[42] [:foo] [\"abc\"]] [[int?] [keyword?] [string?]])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (list 42 (list :foo (list \"abc\"))) (list int? (list keyword? (list string?))))")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b {:c 'foo :d \"abc\"}} {:a int? :b {:c symbol? :d string?}})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b {:c {:d 'foo :e \"abc\"}}} {:a int? :b {:c {:d symbol? :e string?}}})")]
                [:p "Examples of heterogeneous, nested collections."]
                [:pre
                 (print-form-then-eval "(validate-scalar-spec [42 {:a 'foo :b \"abc\"}] [int? {:a symbol? :b string?}])")
                 [:br]
                 (print-form-then-eval "(validate-scalar-spec [42 {:a ['foo {:b (list \"abc\")}]}] [int? {:a [symbol? {:b (list string?)}]}])")
                 [:br]
                 (print-form-then-eval "(validate-scalar-spec [[[[42]]] [[[:foo]]] [[[\"abc\"]]]] [[[[int?]]] [[[keyword?]]] [[[string?]]]])")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? keyword? boolean?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] (pop [int? keyword? boolean?]))")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} {:a int? :b boolean?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} (dissoc {:a int? :b boolean?} :b))")]
                [:p "Second strategy: " [:em "Replace"] " the offending predicate with a more permissive predicate."]
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? keyword? boolean?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? keyword? any?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} {:a int? :b boolean?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} (assoc {:a int? :b boolean?} :b any?))")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} {:a int? :b string?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? (dissoc {:a 42 :b 'foo} :b) {:a int? :b string?})")]
                [:p "Another option involves associating a new value to satisfy the predicate."]
                [:pre (print-form-then-eval "(valid-scalar-spec? (assoc {:a 42 :b 'foo} :b \"foo\") {:a int? :b string?})")]
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo] [int? keyword? string? boolean? ratio? char?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\" true] [int? keyword? string? boolean? ratio? char?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo} {:a int? :b symbol? :c string? :d boolean? :e ratio? :f char?})")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? {:a 42 :b 'foo :d true :f \\c} {:a int? :b symbol? :c string? :d boolean? :e ratio? :f char?})")]
                [:p "Within a seqence, if you'd like to validate some of the values, but skip over others, you have a few options to maintain the correspondence between the data and specification: " [:code "any?"] ", " [:code "(constantly true)"] ", or " [:code "(fn [_] true)"] "."]
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? any? string?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? (constantly true) string?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 :foo \"abc\"] [int? (fn [_] true) string?])")]
               ...
               [:section#collection-validation
                ...
                [:pre
                 (print-form-then-eval "(require '[speculoos.core :refer [validate-collection-spec valid-collection-spec?]])")
                 [:br]
                 (print-form-then-eval "(validate-collection-spec [] [])")
                 (print-form-then-eval "(validate-collection-spec '() '())")
                 (print-form-then-eval "(validate-collection-spec {} {})")
                 (print-form-then-eval "(validate-collection-spec #{} #{})")]
                [:p "There is nothing particularly special about validating an empty collection. In fact, that's a pretty useful task."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [] [empty?])")
                 (print-form-then-eval "(valid-collection-spec? '() '(empty?))")
                 (print-form-then-eval "(valid-collection-spec? {} {:s empty?})")
                 (print-form-then-eval "(valid-collection-spec? #{} #{empty?})")]

                [:h3#coll-vec "Vectors"]
                [:p "Collection predicates apply to the vectors that contain them."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [99 []] [(complement empty?) [empty?]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [1 2 3] [#(= 3 (count %))])")
                 [:br]
                 (print-form-then-eval "(defn length-1? [v] (= 1 (count v)))")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [[1] [2] [3]] [[length-1?] [length-1?] [length-1?]])")]
                [:p "Extending that last example: intervening collection predicates all apply to the root, parent vector."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [[1] [2] [3]] [vector? [length-1?] (complement empty?) [length-1?] any? [length-1?]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [[1] [2] [3]] [[length-1?] vector? (complement empty?) [length-1?] any? [length-1?]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [[1] [2] [3]] [[length-1?] [length-1?] any? (complement empty?) vector? [length-1?]])")]
                ...
                [:pre
                 (print-form-then-eval "(valid-collection-spec? '() '(list?))")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? '(1 2 3) '(#(= 3 (count %))))")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? '((1) (2) (3)) '((length-1?) (length-1?) (length-1?)))")
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 42} {:coll-test map?})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 42} {:coll-test map? :coll-size #(= 1 (count %))})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 42} {:has-key? #(contains? % :a)})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 42 :b 'foo :c 99} {:all-vals-ints? #(every? int? (vals %))})")]

                [:h3 "Sets"]
                [:p "Predicates within a set apply to the set itself as a whole."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? #{1 2 3} #{#(every? int? %)})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? #{1 2 :foo} #{#(every? int? %)})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? #{1 2 3} #{#(every? int? %)
                                                                                #(every? pos? %)})")
                 [:br]
                ...
                [:pre (print-form-then-eval "(valid-collection-spec? #{1 2 3} #{#(every? (complement keyword?) %)})")]
                [:p "For every vector contained in a set, does it contain one integer?"]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? #{:a [1] :b [42] :c [99]} #{(fn [s] (every? #(= 1 (count %)) (filter vector? s)))})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? #{:a [1 2 3] :b } #{(fn [s] (every? #(= 1 (count %)) (filter vector? s)))})")]

                [:h3 "Non-terminating Sequences"]
                [:p "You may supply a (possibly) non-terminating sequence in the data, or in the specification, but not both. During collection validation, the non-terminating sequence is clamped to a length of other corresponding sequence."]
                [:pre
                 (print-form-then-eval "(set! *print-length* 99)")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (range 99) [#(every? int? %)])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (range 99) [#(every? ratio? %)])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (cycle [42 :foo \"abc\"]) [#(valid-scalar-spec? % [int? keyword? string?])])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (iterate #(+ 3 %) 1) [(fn [s] (every? true? (map #(= (+ 3 %1) %2) s (next s)))) (constantly true) (constantly true)])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (repeat :na) [(fn [s] (every? #(= :na %) s)) (constantly true) (constantly true)])")]

                [:h3 "Nested collections"]
                [:p "Same rule, recursively: predicates apply to their parent collection."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [[1] [:a] ['foo]] [[#(every? int? %)] [#(every? keyword? %)] [#(every? symbol? %)]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [[1 22/7] [:a] ['foo]] [[#(every? int? %)] [#(every? keyword? %)] [#(every? symbol? %)]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [11 [22 33 44]] [vector? [#(= 3 (count %))] #(= 2 (count %))])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 11 :b {:c 22}} {:has-keys? #(and (contains? % :a)
                                                                                                           (contains? % :b))
                                                                                          :b {:is-map? map?
                                                                                              :one-MapEntry? #(= 1 (count %))}})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (list 11 (list 22 33)) (list list? (list #(<= 2 (count %))) #(= 2 (count %))))")]
                [:p "Heterogeneous, nested collections work the same way."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [{:a 1 :b 2} 33 {:d 4 :e 5} 66] [#(= 2 (count (filter map? %)))
                                                                                                       {:has? #(contains? % :b)}
                                                                                                       {:has? #(contains? % :d)}
                                                                                                       (fn [v] (every? int? (filter #((complement map?) %) v)))])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a [1 2 3] :b (list \\z \\q \\w) :c {:x \"abc\" :y 42}}
                 ...
                 (print-form-then-eval "(valid-collection-spec? [[[[]]]] [[[[empty?]]]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a {:b {:c {:d {}}}}} {:a {:b {:c {:d {:contains-anything empty?}}}}})")]
                ...
                [:pre
                 ...
                 (print-form-then-eval "(int+string* [1 2 3 'a 'b])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [1 2 3 \"a\" \"b\"] [int+string*])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [1 2 3 'a 'b] [int+string*])")]
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [:a :b :c] [three-kw-and?])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [:a :b] [three-kw-and?])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [:a :b :c 1 2 3] [three-kw-and?])")]
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [1 2 3 \"a\" \"b\" \"c\"] [#(seq-regex % int? :+ string? :*)])")]
                [:p "Exactly three keywords, followed by anything."]
                [:pre (print-form-then-eval "(valid-collection-spec? [:a :b :c 'foo \"bar\" \\z] [#(seq-regex % keyword? 3 any? :*)])")]
                ...
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [[11] [22] [33]] [[#(every? int? %)] [#(= 1 (count %))] [#(not-empty %)]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [[11] [22] [33]] (drop-last [[#(every? int? %)] [#(= 1 (count %))] [#(not-empty %)]]))")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a [1 2 3] :b ['foo 'bar 'baz]} {:a [#(every? int? %)] :b [#(every? int? %)]})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a [1 2 3] :b ['foo 'bar 'baz]}
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [99 {:a 1 :b (list 42 22/7 [\"abc\" {:c [\\z \\q]}])}]
                                                                      [{:b (list [{:c [map?]}])}])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [99 {:a 1 :b (list 42 22/7 [\"abc\" {:c [\\z \\q]}])}]
                ...
                [:pre
                 (print-form-then-eval "(valid-collection-spec? {:a [99 (list {:b [42]})]}
                ...
                [:h3 "Repairing data"]
                [:p "Adjust or remove datums such that the data collection satisfies your specificaiton. Using " [:code "clojure.core"] " functions."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [[1] [2 3] [4 5 6]] [[vector?] [vector?] [map?]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (update-in [[1] [2 3] [4 5 6]] [2] #(zipmap [:a :b :c] %)) [[vector?] [vector?] [map?]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 42 :b {:c 99}} {:b {:has-anything? #(= 0 (count %))}})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (assoc-in {:a 42 :b {:c 99}} [:b] {}) {:b {:has-anything? #(= 0 (count %))}})")]
                [:p "Using Speculoos' " [:code "fn-in*"] " functions."]
                [:pre
                 (print-form-then-eval "(valid-collection-spec? [11 [22 33 [44 55 66 [77]]]] [[[[list?]]]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (update-in* [11 [22 33 [44 55 66 [77]]]] [1 2 3] #(into '() %)) [[[[list?]]]])")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? {:a 42 :b {:c {:d [99 0 'foo]}}} {:b {:c {:d [empty?]}}})")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (dissoc-in* {:a 42 :b {:c {:d [99]}}} [:b :c :d 0]) {:b {:c {:d [empty?]}}})")
               ...
               [:section#function-validation
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-macro-spec? '(infix (2 + 3)) infix-spec)")
                 [:br]
                 (print-form-then-eval "(valid-macro-spec? '(infix (2.2 + 3.3)) infix-spec)")]
                ] ;; end of Function Validation section


               [:section#fn-in
                [:h2 (h2/html [:code "fn*"] " and " [:code "fn-in*"])]
                [:p "Starred functions inspect and manipulate values buried in heterogenous, arbitrarily-nested data structures."]
               ...
               [:section#utility
                ...
                [:h3 "All paths"]
                [:p "Exhaustive sequence of paths of all values in a heterogenous, arbitrarily-nested data structure."]
                [:pre (print-form-then-eval "(speculoos.core/all-paths [11 22 {:a 'foo :b \\z}])")]
                [:h3 "Data from scalar specification"]
                [:pre
                 (print-form-then-eval "(require '[speculoos.utility :refer [data-from-spec spec-from-data basic-collection-spec-from-data all-specs-okay data-without-specs collections-without-specs exercise in? defpred unfindable-generators validate-predicate->generator]])")
                 [:br]
...
;; => [{:collection-predicate map?} (list?) vector?]"]]
                ...
                [:pre (print-form-then-eval "(data-without-specs [11 {:a 22 :b \"abc\"}] [int? {:b string?}])")]
                [:h3 "Collections without a corresponding collection predicate"]
                [:pre (print-form-then-eval "(collections-without-specs [42 {:a (list 22/7)} ['foo]] [vector? {:a (list list?) :is-map? map?}])")]
                [:h3 "Filtering validation results"]
                [:pre (print-form-then-eval "(only-invalid (validate-scalar-spec [11 [\"abc\" [:foo]]] [int? [string? [symbol?]]]))")]
...
;; => [{:path [1], :value no-gen}]"]]
               ...
               [:section#predicates
                ...
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [42 \"abc\" :foo] [int? symbol? keyword?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 \"abc\" :foo] [int? any? keyword?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 \"abc\" :foo] [int? (constantly true) keyword?])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [42 \"abc\" :foo] [int? (fn [_] true) keyword?])")]
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [\"abc123\" \"xyz789\"] [regex-predicate regex-predicate])")
                 [:br]
                 (print-form-then-eval "(only-invalid (validate-scalar-spec [\"abc123\" \"qr89\"] [regex-predicate regex-predicate]))")]
                [:p "Speculoos treats free-floating regexes as predicates."]
                [:pre (print-form-then-eval "(validate-scalar-spec [\"abc123\" \"qr89\"] [#\"\\w{3}\\d{3}\" #\"[qrs]{2}[789]{2}\"])")]
                [:h3 "Sequence regexes"]
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? [1 2 3 :a :b 'foo 'bar 'baz 'qux] [three-ints-two-kw-and-syms?])")
                 [:br]
                 (print-form-then-eval "(def sym-str-opt-int? #(seq-regex % symbol? :. string? :. int? :?))")
                 [:br]
                 (print-form-then-eval "(valid-collection-spec? (list 'foo \"abc\" 42) (list sym-str-opt-int?))")]
                [:h3 "Sets as predicates"]
                [:p "Within a scalar specification, a set creates a predicate that tests for membership."]
                [:pre
                 (print-form-then-eval "(valid-scalar-spec? [:orange] [#{:red :orange :yellow}])")
                 [:br]
                 (print-form-then-eval "(valid-scalar-spec? [:pantheon] [#{:red :orange :yellow}])")]
                ...
                [:pre
                 ...
                 [:br]
                 (print-form-then-eval "(validate-scalar-spec [+ -] [one-something-two-equals-three? one-something-two-equals-three?])")
