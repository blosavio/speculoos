[:section#scalar-validation
 ...
 [:ol
  [:li "Runs " [:code "all-paths"] " on both the data, then the specification."]
 ...
 [:pre
  ...
  [:br]
  (print-form-then-eval "(all-paths [int?])")]
 ...
 [:hr]
 [:p "I am curious to know whether the discussion to this point is sufficient for Clojure programmers to get 80% of their specification and validation work done. 50%? "]
 [:p "Onward…"]
 [:hr]
 ...
 [:p "Three complete pairs of scalars and predicates."
  ...
  [:a {:href "#valid-and-thorough"} "Later"]  ", we'll see that the lone unsatisfied " [:code "char?"] " predicate would result in " [:em "valid"] " operation return " [:code "false"] "."]
