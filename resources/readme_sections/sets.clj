[:section#sets
 ...
 [:p "When using Speculoos, we encounter sets in three scenarios. Well briefly sketch the three scenarios, then later go into the details."]
 ...
 [:p "Mantra #1: Collection validation ignores scalars, so out of all those elements, validation will only be considering the root at path " [:code "[]"] " and the nested set at path " [:code "[1]"] "."]
